...
<div class="p-4 md:p-6">
  <h1 class="text-2xl font-bold mb-6">Cadastro de bancos</h1>
  <x-form wire:submit.prevent="cadastrar" class="flex flex-col gap-4">
    <div class="flex flex-col md:grid md:grid-cols-5 md:items-end gap-4">
      <div class="col-span-3">
        <x-input label="Nome do banco" placeholder="Insira o nome do banco..." wire:model="banco.nome" inline/>
      </div>
    ...
    <div class="flex flex-col md:grid md:grid-cols-2 md:items-end gap-4">
      <div class="col-span-1">
        <x-input label="Agência" wire:model="banco.agencia" placeholder="Insira o número da agência..." inline/>
      </div>
      <div class="col-span-1">
        <x-input label="Conta" wire:model="banco.numero_conta" placeholder="Insira o número da conta..." inline/>
      </div>
    </div>
    <x-textarea rows="5" wire:model="banco.descricao" label="Descrição" placeholder="Insira uma descrição do banco..." inline />
    <x-slot:actions>
      <x-button label="Cancelar" class="btn btn-error" link="{{ route('financas.bancos.listagem') }}" wire:loading.attr="disabled"/>
      <x-button type="submit" label="Cadastrar" class="btn btn-success" wire:loading.attr="disabled" spinner="cadastrar"/>
    </x-slot:actions>
  </x-form>
</div>
