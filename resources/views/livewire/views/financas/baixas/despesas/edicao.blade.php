...
<div class="p-4 md:p-6">
  <p class="font-bold text-2xl">Edição de baixa das despesas</p>
  <x-form wire:submit.prevent="edicao">
    ...
    <div class="flex flex-col gap-4 md:flex-row">
      <div class="flex-1">
        <x-input label="Descrição" wire:model.fill="baixa.descricao" value="{{ $baixaAtual->descricao }}" placeholder="Informe a descrição da baixa..." required />
      </div>
      <div class="flex-1">
        <x-choices
          label="Banco"
    ...
    <div class="flex flex-col gap-4 md:flex-row">
      <div class="flex-1">
        <x-select label="Forma de pagamento" wire:model.fill="baixa.forma_pagamento" value="{{ $baixaAtual->forma_pagamento }}" placeholder="Selecione a forma de pagamento..." :options="auth()->user()->formasPagamento()" required />
      </div>
      <div class="flex-1">
        <x-datepicker label="Data do recebimento" wire:model="baixa.data_baixa" icon="o-calendar" :config="$configuracaoDatetime" required />
      </div>
      <div class="flex-1">
        <x-input label="Valor" wire:model.fill="baixa.valor" prefix="R$" locale="pt-BR" value="{{ $baixaAtual->valor }}" money required />
      </div>
    </div>

    <x-textarea label="Observações" rows="5" wire:model.fill="baixa.observacoes" value="{{ $baixaAtual->observacoes }}" />

    <x-slot:actions>
      <x-button label="Voltar" link="{{ route('financas.baixas.despesas.listagem') }}" class="btn btn-error" />
      <x-button label="Salvar" wire:click="edicao" wire:loading.attr="disabled" spinner="edicao" class="btn btn-success" />
    </x-slot:actions>
  </x-form>
</div>
